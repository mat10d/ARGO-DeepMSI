"""Run the S4 attention-MIL scorer on the Q4 clean cohort.

Requires scripts/build_tilemil_bags.py to have written the tumor tile bags. Writes
results/scorers/clam_tilemil/{slide_scores.csv, metadata.json, metrics.json,
few_shot_curve.csv}. CPU-only (ABMIL on frozen features).
"""
...
from argo_deepmsi.scorers.clam_tilemil import CLAMTileMIL, _write_metrics
...
def main() -> None:
    ...
    print(f"clam_tilemil: auroc={m['auroc']:.4f} spec@sens95={m['spec_at_sens95']:.3f} "
