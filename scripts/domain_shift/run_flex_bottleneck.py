"""Run the S/R FLEX bottleneck scorer on the Q4 clean cohort.

Writes results/scorers/flex_bottleneck/{slide_scores.csv, metadata.json, metrics.json,
few_shot_curve.csv}. CPU-only (batched slide-level features).
"""
...
from argo_deepmsi.scorers.flex_bottleneck import FlexBottleneck, _write_metrics
...
def main() -> None:
    ...
    print(f"flex_bottleneck: auroc={m['auroc']:.4f} spec@sens95={m['spec_at_sens95']:.3f} "
